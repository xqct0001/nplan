...
N-Plan 是一个本地任务理解与任务拆分模块，用于把自然语言请求转换成结构化规划产物。

它刻意保持“只规划、不执行”的边界。N-Plan 不执行任务、不编辑文件、不运行 shell
...
```powershell
n-plan -p "设计一个本地文件整理工具，可以扫描文件、分类，并输出 Markdown 报告"
```
...
部分国产 OpenAI-compatible API 不接受 JSON mode 参数。N-Plan 支持在 Provider
...
N-Plan 采用 Knowledge Catalog / OKF 中适合本地项目的部分：
...
项目知识包位于 [docs/n-agent-knowledge](docs/n-agent-knowledge/)。面向维护者的说明见
