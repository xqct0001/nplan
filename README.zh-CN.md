...
从源码目录使用时，优先走本地启动器。它只要求 Node.js 在 `PATH` 中，不会创建全局命令。
...
PowerShell 不会默认运行当前目录里的命令，所以必须加 `.\`。CMD 可以直接使用 `install` 这类当前目录辅助脚本。

如果你想把当前源码目录安装成全局 `nplan` 命令：
...
`install` 会运行 `npm link`，并确认 npm 已注册全局 link。以后要移除全局命令，运行：
...
如果没有运行 `install`，示例中的 `nplan` 在 CMD 中改用 `nplan.cmd`，在 PowerShell 中改用 `.\nplan.cmd`。
...
CLI 会在不突破规划边界的前提下对齐 Claude Code 的交互形态：无参数进入会话、带引号的 prompt 作为初始任务、`-p` 单次输出、stdin 管道输入、`--continue` / `--resume` 复用本地会话记录。会话记录保存在 `.nplan/sessions/`，该目录已被 git 忽略。
