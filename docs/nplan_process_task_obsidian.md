...
`nplan.cmd setup` 是 PowerShell 中唯一推荐的用户配置入口。它会引导选择 Provider、输入 API Key、获取模型列表并写入 `.nplan/config.toml`。
...
flowchart TD
  ...
  Setup["nplan.cmd setup<br/>src/model-wizard.js"]
...
| `src/model-wizard.js` | `nplan.cmd setup` 引导式配置 |
