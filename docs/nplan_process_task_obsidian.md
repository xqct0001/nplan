...
恢复 session v2 时会重新校验最近及各轮 WorkPlan。有效计划可直接用于 `/步骤`、`/来源` 和 `/导出`；无效或被篡改的计划置为空并提示重新规划，不进入修订摘要、查看或导出。会话文件不保存证据正文、来源内容、绝对路径、API Key 或 Authorization；保存的来源仅包含稳定 `source_id` 与项目相对路径。已有有效 WorkPlan 时直接输入文字或在 `--resume` 后附带文字都会按修订处理，`/新建` 清空该状态。
...
只有 `planned` 状态可派生 WorkPlan 的步骤和整体验收标准；另外两种状态保持为空。WorkPlan 在保存、恢复、渲染与导出前都必须通过结构、依赖、产出和验收校验。
