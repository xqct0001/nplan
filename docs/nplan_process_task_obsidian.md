...
恢复 session v2 时会立即恢复最近的 WorkPlan，因此 `/步骤`、`/来源` 和 `/导出` 可直接使用。会话文件不保存证据正文、来源内容、绝对路径、API Key 或 Authorization；保存的来源仅包含稳定 `source_id` 与项目相对路径。已有 WorkPlan 时直接输入文字按修订处理，`/新建` 清空该状态。
